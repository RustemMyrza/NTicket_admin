    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr>
                        <th>#</th><th>Заголовок</th><th>Партнеры</th><th>Действия</th>
                    </tr>
                </thead>
                <tbody>
                @foreach($partner_blocks as $item)
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="{{ url('/admin/partner-blocks/' . $item->id . '/edit') }}" title="Редактировать"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt" aria-hidden="true"></i> Редактировать</button></a>

                            <form method="POST" action="{{ url('/admin/partner-blocks' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
