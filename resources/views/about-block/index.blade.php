...
@section('content')
    ...
    <div class="card-body">
        ...
        <form method="GET" action="{{ url('/admin/about-block') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr>
                        <th>#</th><th>Описание</th><th>Изображение</th><th>Действия</th>
                    </tr>
                </thead>
                <tbody>
                @foreach($aboutblock as $item)
                    <tr>
                        ...
                        <td><img src="{{\Config::get('constants.alias.cdn_url').$item->image}}" alt="" width="200px;"></td>
                        <td>
                            ...
                            <a href="{{ url('/admin/about-block/' . $item->id . '/edit') }}" title="Редактировать блок"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt" aria-hidden="true"></i> Редактировать</button></a>

                            <form method="POST" action="{{ url('/admin/about-block' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
